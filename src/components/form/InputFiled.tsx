...
const InputField = ({
  ...
  return (
    <div className="mb-3">
      <div>
        {label && (
          <label className="text-[14px] xl:text-[20px] inline-block mb-1 ">{label}</label>
        )}
        <div
          className={
            "flex w-full rounded-[10px]  border border-gray-light bg-white px-4 py-2 text-sm h-12" +
            className
          }
        >
          <input
            ...
            {...props}
          />
          {password && <button type="button" onClick={TogglePassword}>{showPassword ? <Eye/> : <EyeOff />}</button>}
        </div>
      </div>
