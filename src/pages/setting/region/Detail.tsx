...
const RegionDetail = () => {
  ...
  const deleteHandler = async () => {
    const res = await dispatch(region.deleteRegion(regionId) as any);
  ...
  return (
    ...
    {regionData && <div className="flex flex-col space-y-6">
      <div className="flex justify-between items-center mb-[2px]">
        ...
        <p className="text-2xl font-semibold">Region Detail</p>
        <div className="flex items-center text-base font-normal  h-10">
          <p className="py-2 px-2 border-r border-r-gray text-gray">
          Region
          </p>{" "}
          <p className=" py-2 px-2">Region Detail</p>
        </div>
      </div>
      <div className="w-full flex items-center justify-between p-6 bg-white rounded-[10px] drop-shadow">
        <div className="flex items-center justify-center space-x-[84px]">
          <div className="flex flex-col">
            ...
            <p className="text-secondary leading-6">{formatDate(regionData?.created_at)}</p>
          </div>
          <div className="flex flex-col">
            <p className="text-gray leading-6">Created By</p>
            <p className="text-secondary leading-6">{regionData?.created_user?.username || "Unknown"}</p>
          </div>
        </div>
        <div className="flex items-center space-x-6">
          ...
          >
            ...
            <p className="text-[20px] text-white">Edit Region</p>
          </button>
        ...
        <div className="flex space-x-[190px] justify-start w-full ">
          <div className="flex flex-col space-y-4 w-[504px]">
            <div className="h-12 w-full py-3 px-4 flex items-center justify-between">
              ...
              <p className="text-secondary w-[252px]">{regionData?.block?.city?.city_eng}</p>
            </div>
            <div className="h-12 w-full py-3 px-4 flex items-center justify-between">
              <p className="text-gray">Region Name</p>
              <p className="text-secondary w-[252px]">{regionData?.region_eng}</p>
            </div>
      ...
      {isDelete && (
        <ModalComponent
          title="Confirm"
          body={"Are you sure to delete this define region? Please confirm it."}
          open={isDelete}
      ...
      {isSuccess && (
        <AlertModal
          title="Success"
          body={"The region is successfully deleted. Please check into list."}
          open={isSuccess}
