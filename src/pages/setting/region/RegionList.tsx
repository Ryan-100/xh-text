...
const RegionList = () => {
  ...
  const fetchRegionByFilter = async (params) => {
    try {
      const res = await dispatch(region.getAllRegionsByFilter(params) as any);
  ...
  const deleteHandler = async (id) => {
    const res = await dispatch(region.deleteRegion(id) as any);
  ...
  const handleEdit = (id) => {
    navigate("/setting/region/" + id + "/edit");
  };
  const amountColumns: GridColDef[] = [
    { field: "no", headerName: "No.", width: 148 },
    { field: "region_eng", headerName: "Region Name", width: 315 },
    {
      field: "city",
      headerName: "City Branch",
      width: 352,
      renderCell: (params) => {
        return <p className="">{params.row.block.city.city_eng}</p>;
      },
    },
  ];
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          ...
          <div className="text-center">
            <p className="text-2xl font-semibold">Region</p>
          </div>
          <div className="flex items-center text-base font-normal  h-10">
            <p className="py-2 px-2 border-r border-r-gray text-gray">
              Settings
            </p>
            <p className="py-2 px-2">Region</p>
          </div>
        </div>
