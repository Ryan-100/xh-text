...
const RoleDetail = () => {
  ...
  const deleteHandler = async () => {
    const res = await dispatch(role.deleteRole(roleId) as any);
    ...
    <>
      {roleData && (
        <div className="flex flex-col space-y-6">
          <div className="flex justify-between items-center mb-[2px]">
            ...
            <p className="title">Admin Role Detail</p>
          ...
          <div className="w-full flex items-center justify-between p-6 bg-white rounded-[10px] drop-shadow">
            <div className="flex items-center justify-center space-x-[84px]">
              <div className="flex flex-col">
                ...
                <p className="text-secondary leading-6">{formatDate(roleData?.created_at)}</p>
              </div>
              <div className="flex flex-col">
                <p className="text-gray leading-6">Created By</p>
                <p className="text-secondary leading-6">{roleData?.created_user?.username || "Unknown"}</p>
              </div>
            </div>
            <div className="flex items-center space-x-6">
              ...
              >
                ...
                <p className="btn-lg">Edit Admin Role</p>
              </div>
            ...
            <div className="flex space-x-[190px] justify-start w-full ">
              <div className="flex flex-col space-y-4 w-[504px]">
                <div className="h-12 w-full py-3 px-4 flex items-center justify-between">
                  ...
                  <p className="text-secondary w-[252px]">{roleData?.city?.city_eng}</p>
                </div>
                <div className="h-12 w-full py-3 px-4 flex items-center justify-between">
                  <p className="text-gray">Counter</p>
                  <p className="text-secondary w-[252px]">
                  {roleData?.counter?.name}
                  </p>
                </div>
                <div className="h-12 w-full py-3 px-4 flex items-center justify-between">
      ...
      {isDelete && (
        <ModalComponent
          title="Confirm"
          body={
            "Are you sure to delete this define admin role? Please confirm it."
          }
          open={isDelete}
      ...
      {isSuccess && (
        <AlertModal
          title="Success"
          body={
            "The admin role is successfully deleted. Please check into list."
          }
          open={isSuccess}
