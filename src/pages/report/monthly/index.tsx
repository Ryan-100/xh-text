...
const MonthlyReport = () => {
	...
	const fetchReport = async () => {
		try {
			const _data = selectedDate.format("YYYY-MM");
			const startOfMonth = moment(`${_data}-01T00:00:00Z`).toISOString();
			const endOfMonth = moment(`${_data}-01`).endOf("month").toISOString();
			const { data } = await dispatch(
				report.getReport(startOfMonth, endOfMonth) as any
			);
			setData(data);
			const totalIncome = {
				id: new Date(),
